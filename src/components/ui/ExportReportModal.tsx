...
export type ExportStudentStatusType = 'all' | 'active' | 'inactive';

export interface ExportFilterValues {
  format: ExportFormatType;
  feeStatus: ExportFeeStatusType;
  studentStatus: ExportStudentStatusType;
  month: number | null;
  year: number | null;
}
...
export interface ExportReportModalProps {
  ...
  isLoading?: boolean;
  onClose: () => void;
  onExport: (values: ExportFilterValues, actionType: 'download' | 'share') => void;
}
...
const FORMAT_OPTIONS: {
  ...
  description: string;
}[] = [
    {
      ...
      description: 'Formatted student details, attendance & fees',
    },
    {
      id: 'pdf',
      label: 'PDF Document',
      ...
      description: 'Print-ready student report sheet',
    },
    {
      id: 'csv',
      label: 'CSV Data File',
      ...
      description: 'Universal table data for imports & backup',
    },
  ];
...
const STUDENT_STATUS_FILTERS: { id: ExportStudentStatusType; label: string }[] = [
...
export default function ExportReportModal({
  ...
  const [selectedStudentStatus, setSelectedStudentStatus] = useState<ExportStudentStatusType>('active');
      ...
      <View style={sheetStyles.overlay}>
        ...
        >
            ...
            <View className="flex-row items-center justify-between">
              <View className="flex-1 pr-3">
                ...
                <Text className="text-[13px] font-urbanist-medium text-secondary tracking-tight mt-0.5" numberOfLines={1}>
                  {batchName ? `Batch: ${batchName}` : 'All Students Report'}
                </Text>
              </View>
          ...
          >
            ...
            {/* 3. Student Status Filter (Active / Inactive) */}
            <View>
              <View className="flex-row items-center justify-between mb-2">
                <Text className="text-[13px] font-urbanist-bold text-secondary uppercase tracking-wider">
                ...
                {selectedStudentStatus !== 'all' && (
                  <Text className="text-[12px] font-urbanist-bold text-[#4186F7]">
                    {selectedStudentStatus.toUpperCase()}
                  </Text>
                )}
              </View>
              <View className="flex-row gap-2">
                {STUDENT_STATUS_FILTERS.map((st) => {
                  const isSelected = selectedStudentStatus === st.id;
                  return (
                    <TouchableOpacity
                      key={st.id}
                      onPress={() => {
                        ...
                        setSelectedStudentStatus(st.id);
                      }}
                      activeOpacity={0.8}
            ...
            {/* 4. Target Fee Month & Year Filter */}
          ...
          {/* Bottom Action Button Bar with Download and Share Options */}
